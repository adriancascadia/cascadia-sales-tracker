...
export default function PredictiveAnalytics() {
  ...
  const highRiskCustomers = useMemo(() => {
    ...
    return customers
      .map(customer => {
        ...
        const lastVisit = visits?.filter((v: any) => v.customerId === customer.id).sort((a: any, b: any) =>
  ...
  const highValueCustomers = useMemo(() => {
    ...
    const customerMetrics = customers.map(c => ({
      id: c.id,
      totalSpent: orders.filter(o => o.customerId === c.id).reduce((sum, o) => sum + (parseFloat(o.totalAmount) || 0), 0),
      orderCount: orders.filter(o => o.customerId === c.id).length,
      monthsActive: 6, // Simplified
    }));
    const highValueIds = identifyHighValueCustomers(customerMetrics.map(m => ({ ...m, id: m.id.toString() })), 80);
